var style = new function () {
	...
	var getPropertyValue = function(k,v) {
		...
		switch(k) {
			...
			case 'background-image':
				if(base.selector.selector.indexOf('galProjectBg')>-1) {
					base.property.bgUrl = (v.indexOf('googleusercontent') > -1) ? v : 'none';
				} else base.property.bgUrl = $(base.selector).css('background-image');	
				break;
			...
			case 'font-size':
				...
				base.property.txSize = (_txSize!=null) ? $(base.selector).css('font-size').replace('px','') : '';
				break;
	...
	this.setCssProperty = function(j,selector,key,val) {
		...
		if(selectEL == 'el-menu') {	
			...
			if (	key=='border-color' &&
					selectEL != '' && 
					selector == '.userEL' + $('.'+selectEL).attr('data-id') + ' .carousel-indicators li'
			) {
			...
			} else if(selector.indexOf('.figure.like') > -1) {
				...
				$.each(j['children'],function(i,v){
					if(i.indexOf('@media') > -1 && typeof j['children'][i]['children'][selector] != 'undefined') {
						j['children'][i]['children'][selector]['attributes'][key] = val;
					}
				});
			}
	...
	this.deleteMediaCssProperty = function(j,selector,media) {
		...
		$.each(tmp_media_size_selector, function(tmp_idx, tmp_selector) {
			if(	typeof j['children'][tmp_selector] != 'undefined' && 
				...
				$.each(selector, function(tmp_idx2, tmp_selector2) {
					if(typeof j['children'][tmp_selector]['children'][tmp_selector2] != 'undefined') delete j['children'][tmp_selector]['children'][tmp_selector2];
				});
			}
		});
		return j;
	...
	this.selectGalleryCategoryColorSetForm = function(cls,fn,el,elname,colortype) {
		...
		var now_palette = '\
						<div class="color-line cg-font">\
							...
							<div class="active-color ' + (active_selector ? '' : 'disabled') +'">\
								<span class="color-picker">\
					...
					<div class="dropdown-menu">\
						<div class="color-picker-wrap">\
							<div class="header-line">\
								...
								<div class="active_color ' + (active_selector? '' : 'disabled') +'">' + $.lang[LANG]["editor.gallery.category.rollover"] + '</div>\
							</div>\
							' + now_palette + '\
						</div>\
					</div>\
	...
	this.selectFontSelectForm = function(cls,fn,elname) {
        ...
        <div class="btn-group">\
            ...
            <!--화살표 svg 백업\
            <button type="button" class="btn btn-default dropdown-toggle font-preview" data-toggle="dropdown"><span class="' + cls + '-active" style="font-family:\'' + fn + '\'">' + fn_name + '</span> <svg viewBox="0 0 8 4" width="8" height="4"><polygon points="0 0 4 4 8 0 "/></svg></button>\
            -->\
            <ul class="dropdown-menu scrollable-menu" role="menu">\
		        ' + str + '\
            </ul>\
	...
	this.menuColorSetForm = function(j,selector) {
		...
		var getColorBox = function(menu_type, css_type, color_set) {
			var mode = ['default','hover','active'],
				...
				check_return = false,
				css_obj = getStyleText(),
				color_val = getColorAndOpacity('.'+selectEL, css_obj, 'background-color');				
			...
			if(menu_type == 'menublock') {
				...
				str = '\
							<div class="menu-bg color-box '+menu_bg_transparent+'" data-ctype="bg">\
			...
			mode.forEach(function(i,v) {
				var class_opacity = (tpl_color_opacity[menu_type+'_'+i+'_'+css_type] == 0) ? 'transparent' : '',
					class_transprent = ( color_val['op_percent']=='transparent'
				...
				if((menu_type == 'fixbtn' || menu_type == 'cart') && typeof tpl_fixbtn_color[css_type+'_'+i]=='undefined') {
					check_return = true;
					return str;
				}

				var tpl_cstate = ((menu_type == 'fixbtn' || menu_type == 'cart') && v < 2) ? "true" : tpl_color_state[menu_type+'_'+i+'_'+css_type],
					tpl_opacity = ((menu_type == 'fixbtn' || menu_type == 'cart') && v < 2) ? "true" : tpl_color_opacity[menu_type+'_'+i+'_'+css_type],
					tpl_bgColor = ((menu_type == 'fixbtn' || menu_type == 'cart') && v < 2) ? tpl_fixbtn_color[css_type+'_'+i]['colorVal'] : tpl_color_hex[menu_type+'_'+i+'_'+css_type],
					tpl_selector = ((menu_type == 'fixbtn' || menu_type == 'cart') && v < 2) ? tpl_fixbtn_color[css_type+'_'+i]['selector'] : tpl_color_selector[menu_type+'_'+i],
					tpl_pn = ((menu_type == 'fixbtn' || menu_type == 'cart') && v < 2) ? 'fixbtn' : pn;

				str = str + '\
							<div class="color-box '+class_opacity + class_transprent + class_bg_img +'" data-type="'+ menu_type + '" data-mode="'+ i + '" data-ctype="'+ css_type + '" data-cstate="' + tpl_cstate + '"  data-toggle="tooltip" data-placement="bottom" data-html="true" data-original-title="'+transparent_tooltip + '">\
								<span class="color-picker">\
									<span class="color tooltips ctrl-menu-color-picker" el="elmenu-color-edit" data-selector="' + tpl_selector + '" pn="'+tpl_pn+'" data-toggle="tooltip" data-placement="top" data-original-title="color" data-opacity="'+tpl_opacity+'" style="background-color: '+tpl_bgColor+'"></span>\
								</span>\
								' + etc_str + '\
							</div>\
							';
			...
			$.each(root_group, function(i, r_k) {
				...
				root_str += '\
					<div class="color-box rv ' + r_v_transparent + r_v_block_transparent + r_v_block_bgurl + '" ' + color_box_tooltip + '>\
						<span class="color-picker">\
							<span class="color tooltips root-menu-color-picker" el="' + selectEL + '" selector=".' + root_selector + '" data-root="'+r_k.replace(/\_/gi,'-')+'" data-init="' + ((r_v_empty) ? init_key : '') + '" data-toggle="tooltip" data-placement="top" title="color" style="background-color: ' + r_v + ';"></span>\
						</span>\
					</div>\
				';
			});
// data-ti="default" data-tp="cursor-color" 
			return root_str;
		}
