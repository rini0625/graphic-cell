...
$(function() {
	...
	$.fheader = {
		...
		position: function() {
			...
			if(checkEllist) {
				...
				var delay_time = (PAGE_MODE == 'c') ? 1300 : 0;
				setTimeout(function() {
	...
	$.moption = {
		...
		drawClDropFixedmenu: function(pcNav) {
			...
			else pcNav.find('.navbar-header .micon').before(drop_fixed_menu);
		},
	...
	$.psearch = {
		...
		makeSiteSEARCH: function() {
			...
			var pc_nav = $('.el-menu header.navbar #fixed-menu, .el-menu header.navbar #fixed-menu-sub'),
				...
				search_icon = (checkFixTypeMenu) ? clSVG('fixbtn_search04',28,28) : '<i class="cl-icon cl_icon_magnifier" aria-hidden="true"></i>';
...
// class="dashboard" href="javascript:;" data-
	$('.siteUM .join, .siteUM .login, .siteUM .forgot-id, .siteUM .forgot-pw, .siteUM .dashboard, .siteUM .mypage, .siteUM .logout, .siteUM-dmenu-wrap .mypage, .siteUM-dmenu-wrap .logout, .cl-fixed-menu#fixed-menu .logout.cl-umMenu').live({
		click: function(e) {
			e.preventDefault();
						...
						if(checkEmojis(emoji_val)) error_str = $.lang[LANG]['config.unable.emoji'];
							...
							if(!regexp[type].test(val)) error_str = $.lang[LANG]['siteum.regexp.check.'+type];
								...
								var checkDuplicate = (	event_type == 'blur' &&
														...
														$(input).attr('data-prev').toLowerCase() != val.toLowerCase()
													) ? true : false;
