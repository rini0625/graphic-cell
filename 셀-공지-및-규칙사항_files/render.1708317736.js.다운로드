...
var RENDER = {
    init: function (property) {
	            ...
	            History.Adapter.bind(window,'statechange',function(){
			    	...
			    	if(pageMove) {
			    		...
			    		var menu_name = (b.VIEW) ? b.PAGE.split(',') : [b.PAGE],
			    			tmp_url = (is_templates) ? '/render' : '',
			    			pagemenu_link = '#tpl-menu.nav li a[href="'+tmp_url+'/' + menu_name[0] + '"]';
		...
		var showPage = function (css, style, tag, prop) {
		    ...
		    if(b.HEADER && !$('header').hasClass('sidebar')) $('header.navbar').addClass('transparent');
		...
		var loadPush = function(data) {
			...
			function drawData(idx, val) {
				...
				if(val.type == 'gallery' && val.type2 == 'premium' && (!is_templates && (!b.VALIDPLAN || b.VALIDTYPE == 'PK'))) {
					tag =  '<div class="' + val.elname + ' hide"></div>';
				} else if(val.type == 'gallery' || val.type == 'shopping') {
					$('#el-empty').append($(tag));
				...
				//라이브 백업 if(val.type == 'form') {
				if(val.type == 'form') {
					if(!is_templates && (!b.VALIDPLAN || b.VALIDTYPE == 'PK')) {
						$tag.html('').addClass('hide');
				...
				//premium block : 요금제 없을 시 hide
				var isPremiumBlock = (($.inArray(val.type, ['contact','contents','showcase','video']) > -1 && val.type2 == 'premium') ||
							   		  ($.inArray(val.type, ['sns','forum']) > -1 && val.feature == 'premium')) ? true : false;
		        if(isPremiumBlock && (!is_templates && (!b.VALIDPLAN || b.VALIDTYPE == 'PK'))) $tag.html('').addClass('hide');
		...
		var elFooter = function() {
			var error_style = '\
				...
				.error_404.new_design h1 { margin-top:50px; font-size:60px; color:#2d343e; font-family:Arial; font-weight: bold; }\
		...
		function setDefaultForm(el) {
            ...
            if($(el).attr('data-captcha') != 'false' && $(el).find('input.input-captcha').length > 0) {
		...
		//menu click
		$('#tpl-menu.nav > li:not(.siteUM,.siteLANG) a:not([href="javascript:;"]):not([href*="flink@"]),.cl-moption > .cl-moption-col > .cl-moption-row > a:not([href="javascript:;"]):not([href*="flink@"]),.el-menu header.navbar a#site-home').live({
		    click : function(e) {
		    	e.preventDefault();
		...
		// menu click (onepage)
		$('.nav li a[href*=#]:not([href="javascript:;"]), .cl-moption > .cl-moption-col > .cl-moption-row > a[href*=#]:not([href="javascript:;"])').live({
		    click : function(e) {
				if(typeof e.target.className == 'string' && e.target.className.indexOf('fa') > -1 && $(this).closest('header').hasClass('navbar-simple')) {
					e.preventDefault();
					return false;
		...
		$('.gallery-category-nav li a, .gallery-sort-nav li a').live({
		    click: function() {
				...
				if($galleryEL.is('[data-type2="premium"]')) setGalleryJS('change-loop',$galleryEL);
		...
		$(':not(.review-sample-block) .review-sort-nav li a').live({
		    click: function() {
            	...
            	$.reviewContents.update(review_block_id, gallery_data, orderby);
		    }
		});
		...
		$(document).on('click', 'ul li.siteSEARCH > a.psearch-btn, \
		...
		$('#mobile-nav ul li.menu-has-children i').off('click').on('click', function(e) {
		...
		$('a[attr-flink], .fr-custom-button[attr-flink], .fr-fic[attr-flink]').live({
			click : function(e) {
				...
				e.preventDefault();
				if(!property.VALIDPLAN || property.VALIDTYPE == 'PK') return false;
		...
		$('.element .gallery-loadmore').live({
		    click: function(e) {
				...
				if($galleryEL.is('[data-type2="premium"]')) setGalleryJS('add-loop',$galleryEL);
...
``
		...
		checkSiteLock.then(function() {
			isMenuLock(function() {
				...
				$.each(contents, function(i,v) {
		            ...
		            //premium block : 요금제 없을 시 hide		            
		            var isPremiumBlock = (($.inArray(c.type, ['video','contact','contents']) > -1 && c.type2 == 'premium') ||
								   		  ($.inArray(c.type, ['sns','forum']) > -1 && c.feature == 'premium') || 
										  (c.type == 'showcase' && c.feature == 'premium')) ? true : false;
			        // if(isPremiumBlock && (!b.VALIDPLAN || b.VALIDTYPE == 'PK')) $el.html('').addClass('hide');
			        if(isPremiumBlock && (!is_templates && (!b.VALIDPLAN || b.VALIDTYPE == 'PK'))) $el.html('').addClass('hide');
	...
	syncUpdate : function($el,data) {
		...
		if(typeof data.sync !='undefined' && typeof data.sync.list != 'undefined' && (data.sync.list).length) {
			...
			$.each(data.sync.list, function(i,v) {
				...
				$item.find('.brand').text(v.brand_name);
				$item.find('.name').text(v.title);
				$item.find('.price').text('₩'+number_format(v.price));
                if(imgbg_check) $item.find('.img-bg').attr('style','background-image: url("' + v.image + '"');
				if(v.sale_rate > 0) $item.find('.sale').text(v.sale_rate + '%');
                else $item.find('.sale').addClass('hidden');
	...
	galleryUpdate : function(el,data) {
    	var is_templates = checkTemplateSite(this.b.SID);
		if(el.type == 'gallery' && el.type2 == 'premium' && (!is_templates && (!this.b.VALIDPLAN || this.b.VALIDTYPE == 'PK'))) {
			$('.element[data-id="' + el.seq + '"]').html('').addClass('hide');
			return false;
		}
		...
		if($galleryEL.is('[data-type2="premium"]')) setGalleryJS('load-loop',$galleryEL);
	    ...
	    if(type == 'F') {
		    ...
		    $.ajax({
				...
				success: function (fr_data) {
					...
					if(typeof fmConf.fm_settings.comment_display == 'undefined') {
...
var aosAddblock = function(site_settings,settings,$el) {
	if($el.is('[data-type="gallery"][data-type2="premium"]')) {
...
var allProductSortNav = function(elsettings, total) {
    ...
    var html = '\
	    <div class="allProducts-sort-wrap">\
...
\
			...
			if(property.CERT_USE == 'Y'){
				if($.inArray(property.PAGE, ['_register', '_login', '_forgot', '_mypost', '_myinfo']) == -1 && seg[1] == '_login') {
					...
					if(typeof cert_popup !== 'undefined' && cert_popup == 'true' && data.cert == 'N') {
						...
						}, $.lang[LANG]['siteum.cert.identify.colse.btn'], $.lang[LANG]['siteum.cert.identify.verification.btn'],'modal-dialog cl-cmmodal cl-s-btn w560 cl-p70 cl-modal cl-close-btn user-not-cert-modal', false, function(){
...
// 	for (var i=0; i<$('.logoSlide').length; i++) {
// 		console.log('.'+$('.logoSlide').eq(i).attr('class').split(' ')[0]);
// 		slider('.'+$('.logoSlide').eq(i).attr('class').split(' ')[0]);
// 	}
// 	for (var i=0; i<$('.reviewSlide').length; i++) {
// 		console.log('.'+$('.reviewSlide').eq(i).attr('class').split(' ')[0]);
// 		slider('.'+$('.reviewSlide').eq(i).attr('class').split(' ')[0]);
// 	}
// });
